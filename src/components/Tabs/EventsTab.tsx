...
import { Calendar, Edit2, MapPin, Plus, RefreshCw, Save, Search, Settings2, Trash2 } from 'lucide-react';
import { type DbEvent, type DbSiteContent } from '../../lib/mockData';
...
function clampHomeEventsLimit(value: string | number | null | undefined) {
  const parsed = typeof value === 'number' ? value : Number.parseInt(value || '', 10);
  if (!Number.isFinite(parsed)) return 3;
  return Math.min(5, Math.max(1, parsed));
}

export default function EventsTab({ events, searchQuery, setSearchQuery, isLoadingData, openCreateModal, openEditModal, openDeleteModal, siteContent, onUpdateContent }: EventsTabProps) {
  const savedLimit = clampHomeEventsLimit(siteContent.find((item) => item.key === 'events_home_limit')?.value);
  ...
  const handleSaveLimit = () => {
    const clamped = clampHomeEventsLimit(homeLimit);
    setHomeLimit(String(clamped));
    onUpdateContent('events_home_limit', String(clamped), String(clamped));
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-200">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1 leading-relaxed">Jumlah event pada section Event Terkini dibatasi 1 sampai 5. Halaman arsip event tetap menampilkan semua event.</p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Maks tampil</label>
            <input type="number" min={1} max={5} value={homeLimit} onChange={(e) => setHomeLimit(e.target.value)} onBlur={() => setHomeLimit(String(clampHomeEventsLimit(homeLimit)))} className="w-24 px-3 py-2 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900/10" />

...
import { type DbLandingPortfolioItem } from '../../lib/mockData';

interface PortfolioTabProps {
  portfolioItems: DbLandingPortfolioItem[];
  searchQuery: string;
  setSearchQuery: (query: string) => void;
  isLoadingData: boolean;
  openCreateModal: () => void;
  openEditModal: (item: DbLandingPortfolioItem) => void;
    ...
    <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm space-y-6 select-none glass-card">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            type="text"
            placeholder="Cari judul/medium/tahun..."
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-gray-500 font-semibold text-xs uppercase tracking-wider">
                ...
                <th className="p-4">Karya & Tahun</th>
                <th className="p-4">Medium & Teknik</th>
                <th className="p-4">Grid Class</th>
            ...
            <tbody className="divide-y divide-gray-50 text-gray-700">
              {filteredItems.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50/50 transition">
                  ...
                  <td className="p-4">
                    <img src={item.image} className="w-12 h-12 object-cover rounded-xl border border-gray-100 bg-gray-50" alt="" />
                  </td>
                  <td className="p-4">
                    <div className="font-semibold text-gray-950 text-sm leading-snug">{item.title}</div>
                    <div className="text-xs text-gray-400 font-semibold mt-0.5">Tahun {item.year}</div>
                  </td>
                  <td className="p-4 text-gray-500 font-medium text-xs leading-normal">
                    <div>{item.medium}</div>
                    <div className="text-gray-400 text-[10px]">{item.technique}</div>
                  </td>
                  <td className="p-4">
                    <span className="font-mono text-xs px-2 py-1 bg-gray-50 border border-gray-100 rounded-lg text-gray-500">
                      {item.gridClass}
                    </span>
                  </td>
                  <td className="p-4">

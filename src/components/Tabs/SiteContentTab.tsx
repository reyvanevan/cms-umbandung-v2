...
interface SiteContentTabProps {
  ...
  category?: 'beranda' | 'visi_misi' | 'tata_kelola' | 'kurikulum' | 'tugas_akhir' | 'capstone' | 'spm' | 'kerjasama' | 'kkn' | 'statistik' | 'alumni' | 'kegiatan_dosen' | 'kegiatan_mahasiswa' | 'publikasi' | 'dosen' | 'global';
  hideHeader?: boolean;
  dosenList?: DbDosen[];
...
export default function SiteContentTab({
  ...
  const [activeCategory, setActiveCategory] = useState<'beranda' | 'visi_misi' | 'tata_kelola' | 'kurikulum' | 'tugas_akhir' | 'capstone' | 'spm' | 'kerjasama' | 'kkn' | 'statistik' | 'alumni' | 'kegiatan_dosen' | 'kegiatan_mahasiswa' | 'publikasi' | 'dosen' | 'global'>(category || 'beranda');
  ...
  const getCategoryForKey = (key: string) => {
    ...
    if (key.startsWith('spm_')) return 'spm';
  ...
  const CATEGORIES = [
    ...
    { id: 'spm', name: 'Sistem Penjaminan Mutu', icon: FileText, desc: 'Konten halaman kebijakan, siklus, standar, dan dokumen mutu.' },
          ...
          {activeSubSection === 'Ringkasan Landing Page' && !searchQuery ? (
            ...
            subsections.map((subName) => {
              ...
              if (!searchQuery && subName === 'Sistem Penjaminan Mutu') {
                return <div key={subName}>{renderSimplePageIntroEditor('Sistem Penjaminan Mutu', 'Atur heading, overview, kebijakan, siklus PPEPP, standar, peran, dan CTA dokumen mutu.', ['spm_heading', 'spm_description', 'spm_overview_title', 'spm_overview_desc', 'spm_policy_title', 'spm_policy_desc', 'spm_cycle_items', 'spm_standards', 'spm_roles', 'spm_document_url', 'spm_cta_label'], 'section:spm')}</div>;
              }

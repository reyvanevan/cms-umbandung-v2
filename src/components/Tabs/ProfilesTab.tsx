...
import { type DbKurikulumProfile } from '../../lib/mockData';

interface ProfilesTabProps {
  profiles: DbKurikulumProfile[];
  searchQuery: string;
  setSearchQuery: (query: string) => void;
  isLoadingData: boolean;
  openCreateModal: () => void;
  openEditModal: (item: DbKurikulumProfile) => void;
    ...
    <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm space-y-6 select-none glass-card">
      ...
      {isLoadingData ? (
        <div className="py-20 flex flex-col items-center justify-center gap-3 text-gray-400">
          ...
          <p className="text-sm">Memuat data profil lulusan...</p>
        </div>
      ) : (
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-gray-500 font-semibold text-xs uppercase tracking-wider">
                <th className="p-4" style={{ width: '250px' }}>Profil Lulusan (Bilingual)</th>
                <th className="p-4">Deskripsi / Peran Kerja</th>
              ...
              {filteredProfiles.length === 0 && (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-gray-400 text-sm">
                    Tidak ada profil lulusan ditemukan.
                  </td>
                </tr>
              )}
            </tbody>

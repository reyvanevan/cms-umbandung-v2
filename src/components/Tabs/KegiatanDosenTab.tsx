...
import { type DbKegiatanDosen } from '../../lib/mockData';

interface KegiatanDosenTabProps {
  kegiatanList: DbKegiatanDosen[];
  searchQuery: string;
  setSearchQuery: (q: string) => void;
  isLoadingData: boolean;
  openCreateModal: () => void;
  openEditModal: (item: DbKegiatanDosen) => void;
  openDeleteModal: (id: string) => void;
}

export default function KegiatanDosenTab({ kegiatanList, searchQuery, setSearchQuery, isLoadingData, openCreateModal, openEditModal, openDeleteModal }: KegiatanDosenTabProps) {
  const filtered = kegiatanList.filter(item => {
    if (!searchQuery) return true;
    const q = searchQuery.toLowerCase();
    return item.title.toLowerCase().includes(q) || item.location.toLowerCase().includes(q) || item.date_text.toLowerCase().includes(q);
  });
    ...
    <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm space-y-6 select-none glass-card">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="relative w-full sm:max-w-xs">
          ...
          <input type="text" placeholder="Cari kegiatan dosen..." className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-gray-200 focus:bg-white transition" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-gray-500 font-semibold text-xs uppercase tracking-wider">
                <th className="p-4">Judul Kegiatan (Bilingual)</th>
                <th className="p-4" style={{ width: '140px' }}>Tanggal</th>
                <th className="p-4" style={{ width: '180px' }}>Lokasi</th>
            ...
            <tbody className="divide-y divide-gray-50 text-gray-700">
              {filtered.map(item => (
                <tr key={item.id} className="hover:bg-gray-50/50 transition">
                  ...
                  <td className="p-4 text-gray-600 text-xs font-medium">{item.date_text}</td>
                  <td className="p-4 text-gray-500 text-xs">{item.location}</td>
                  <td className="p-4 text-gray-500 text-xs leading-relaxed max-w-xs">
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={5} className="p-8 text-center text-gray-400 text-sm">Tidak ada kegiatan dosen ditemukan.</td></tr>
              )}
            </tbody>
          </table>
        </div>
